...
from DifferenceFormulas import fd
...
def evalJ(x): 
    ...
    f1 = 4*x[0]**2 + x[1]**2 -4
    f2 = x[0] + x[1] - math.sin(x[0] - x[1])
    h1 = 10e-7*x[0]
    h2 = 10e-7*x[1]

    J = np.array([[(f1(x[0] + h1) - f1(x[0])) / h1, (f1(x[1] + h2) - f1(x[1])) / h2],
                  [(f2(x[0] + h1) - f2(x[0])) / h1, (f2(x[1] + h2) - f2(x[1])) / h2]])
    return J
